...
export default function VerificationCode() {
  ...
  const { loading, userOtp, user, isForgotEmail, forgotEmail } = useSelector(state => state.auth);
  ...
  const handleClick = async (e) => {
    ...
    try {
      ...
      // for forgot password :)
      if (isForgotEmail === true) {
        if (verifyForgotPasswordEmail(forgotEmail) === false) {
          navigate('/forgotPassword');
          toast.error('Please Enter Your Valid Email');
          return;
        }
        ...
        await dispatch(verifyForgotPasswordData({ forgotEmail, userOtp }))
          .then((res) => {
